function maxpop = EntCDP_one_matlab(A,k,exclusion)
%
% This function uses genetic algorithm to identify driver gene sets in cancer A.
%
...
for i=1:num
    ...
    %A_index_sum{i}=sum(A_index{i},2);
    %w(i)=(sum(A_index_sum{i}>0))/m(i);
    f=f+(2*sum(A_index_sum{i}>0)-sum(A_index_sum{i}))*entropy_simp(w);%/m(i);(min(m)/m(i));
end
